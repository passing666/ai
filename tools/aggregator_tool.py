...
async def _call_provider(
    module, q: str, limit: int, timeout: float
    ...
    try:
        ...
        if asyncio.iscoroutinefunction(func):
            ...
            res = await asyncio.wait_for(
                loop.run_in_executor(None, func, q, limit), timeout=timeout
            )
        if res is None:
            res = []
    except Exception:
...
async def search(
    q: str, limit: int = 6, provider_timeout: float = DEFAULT_PROVIDER_TIMEOUT
    ...
    tasks = [
        asyncio.create_task(_call_provider(m, q, limit, provider_timeout))
        for m in providers_modules
    ]
    results = []
    if tasks:
...
async def aggregate_search(
    query: str, max_results: int = 6, provider_timeout: float = DEFAULT_PROVIDER_TIMEOUT
):
...
def search_sync(
    ...
    return asyncio.get_event_loop().run_until_complete(
        search(q, limit=limit, provider_timeout=provider_timeout)
    )
